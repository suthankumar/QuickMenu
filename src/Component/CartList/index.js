        ...
        <div id = {"diner"+idx} onClick={()=>UpdateOrderList(idx)} className={`${selectedDine == idx ? "border-l-8  border-red-500 shadow-2xl" : ''}  my-2 bg-white hover:bg-gray-100`}>
            <div className={`${selectedDine == idx ? "bg-red-100" : ''}  p-2 flex `}>
                ...
                <div className="flex w-18 font-medium items-end">
                    <div onClick={()=>UpdateOrderList(idx)} className="w-4 mx-2 hover:bg-green-200 rounded-full cursor-pointer text-green-700">
                    ...
                    {order.list.length > 1 && <div onClick={()=>UpdateOrderList(idx, true)} className="w-4 h-4 hover:bg-red-200 rounded-full cursor-pointer text-red-700">
