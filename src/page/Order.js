...
export default function Order() {
  ...
  useEffect(() => {
      ...
      (function (hash) {
        return function (r, s) {
          s.meal.map((h) => {
            ...
            hash[h.course].id[h.selected] =
              (hash[h.course].id[h.selected] || 0) + 1;
    ...
    stockTaken.forEach((e) => {
      ...
      for (const [key, value] of Object.entries(e.id)) {
        menu[e.course].map((o, i) => {
          ...
          o.id == key
            ? ((meal.stock = orgStock - value), updatedMeal.push(o.id))
            : !updatedMeal.includes(o.id)
            ? (meal.stock = orgStock)
            : null;
        });
      }
    ...
    let curtDiner = order.list[selectedDine].meal,
      salmonFltIndex = menu["mains"].findIndex(
        (h) => h.name == "Salmon fillet"
      ),
      prawnIndex = menu["starters"].findIndex(
        (h) => h.name == "Prawn cocktail"
      ),
      prawn = tempList["starters"][prawnIndex],
      salmonFlt = tempList["mains"][salmonFltIndex];
    if (prawnIndex != -1 && salmonFltIndex != -1) {
      if (
        curtDiner?.filter((m) => m.course == "mains")[0]?.selected ==
        menu["mains"][salmonFltIndex]?.id
      ) {
        //7
        prawn.allow = false;
        salmonFlt.allow = true;
      } else if (
        curtDiner?.filter((m) => m.course == "starters")[0]?.selected ==
        menu["starters"][prawnIndex]?.id
      ) {
        //4
        prawn.allow = true;
        salmonFlt.allow = false;
      } else {
        prawn.allow = true;
        salmonFlt.allow = true;
      }
  ...
  const ValidateDinerMeal = () => {
    if (
      order.list[selectedDine]?.meal.some((g) => g.course == "mains") &&
      order.list[selectedDine].meal.length >= 2
    ) {
      return true;
    } else {
      alert(
        "Please select aleast 2 course for current diner- one course should be main "
      );
      return false;
    }
  };
  ...
  return (
    <div className="h-screen">
      <Header />
      {order.paid && (
        <div className="mx-auto m-10 px-12  flex h-full justify-center">
          <div className="flex flex-col ">
            ...
            <Link
              to="/"
              className="mx-14 mt-5 flex text-red-400 hover:text-red-500 align"
            >
              <svg
              ...
              <div className="text-gray-400 mx-2 text-s flex self-center">
                Back to Homepage
              </div>
            </Link>
          </div>
            ...
            <div className="grid grid-cols-3 gap-4 ">
              <div className="col-span-2 bg-red-50 p-3 rounded-md ">
                <div className="flex space-x-4 ">
                  <h1 className="flex-auto text-2xl font-semibold p-3">Menu</h1>
                </div>
                <CourseTab
                  menu={menu}
              ...
              <div className="bg-red-700 p-3 rounded-md relative">
                <div className="flex space-x-4 ">
                  ...
                  <div className="flex">
                    <button
                      ...
                      className="flex items-center justify-center px-2 rounded-md font-semibold text-white bg-red-600 hover:bg-white hover:text-red-600"
                    >
                      + Add New diner
                    </button>
                  </div>
                ...
                <div className="absolute bottom-10 right-0 p-4 justify-center flex w-full bg-opacity-90 ">
                  <button
                    onClick={() => pay()}
