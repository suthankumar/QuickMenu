...
function MealCard({ ml, selectedCourse, selectedDine, order, setOrder }) {
  ...
  const onSelect = (id, unavalible) => {
    if (!unavalible) {
      ...
      if (index != -1) {
        list[index].selected = id;
      } else {
        list.push({ course: selectedCourse, selected: id });
      }
  ...
  return (
    <div
      id={ml.name}
      onClick={() => onSelect(ml.id, ml.stock <= 0 || ml.allow == false)}
      className={`${
        order.list[selectedDine]?.meal.filter(
          (f) => f.course == selectedCourse
        )[0]?.selected == ml.id
          ? "bg-red-200 border-2 border-red-500"
          : "bg-white"
      } w-60 py-4 px-8 
            ${
              ml.stock <= 0 || ml.allow == false
                ? "opacity-70"
    ...
    >
      ...
      <div className="flex justify-end mt-4">
        <div href="#" className="text-xl font-medium text-red-600">
          £{ml.price}
        </div>
      </div>
      <div
        className={`${
          ml.stock <= 0 || ml.allow == false ? "text-red-600" : "text-gray-400"
        }`}
      >{`${
        ml.stock == 0
          ? `Out of Stock`
